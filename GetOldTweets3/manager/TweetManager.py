    ...
    def getJsonResponse(tweetCriteria, refreshCursor, cookieJar, proxy, useragent=None, debug=False):
        ...
        while count <= max_tries:  # try 10 times
            try:
                ...
                if count < max_tries:
                    print('['+str(datetime.now())+'] Error occurred, retrying in '+str(delay)+' seconds ('+str(count)+'/'+str(max_tries)+')')
                    log_file.write('['+str(datetime.now())+'] Error occurred, retrying in '+str(delay)+' seconds ('+str(count)+'/'+str(max_tries)+')\n')
                    time.sleep(delay)
                    delay += 100
                else:
                    # If trying 3rd time and still error?? 
                    # Just throw the error- we don't have anything to hide :)
                    print('['+str(datetime.now())+'] Error occurred more than number of tries: '+str(e))
                    log_file.write('['+str(datetime.now())+'] '+str(tweetCriteria.since)+'Error occurred more than number of tries: '+str(e))
