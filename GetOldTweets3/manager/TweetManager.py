...
class TweetManager:
    ...
    def getJsonResponse(tweetCriteria, refreshCursor, cookieJar, proxy, useragent=None, debug=False):
        ...
        #Request handling 429
        log_file = open("./corona/log.txt","a") 
        delay = 300
        count = 1
        max_tries = 10
        while count <= max_tries:  # try 10 times
            try:
                ...
                if count < max_tries:
                    print('['+str(datetime.datetime.now())+'] Error occurred, retrying in '+str(delay)+' seconds ('+str(count)+'/'+str(max_tries)+')')
                    log_file.write('Proccess '+str(tweetCriteria.since)+'/'+str(tweetCriteria.until)+'\n')
                    log_file.write('['+str(datetime.datetime.now())+'] Error occurred, retrying in '+str(delay)+' seconds ('+str(count)+'/'+str(max_tries)+')\n\n')
                    time.sleep(delay)
                    delay += 100
                else:
                    # If trying 3rd time and still error?? 
                    # Just throw the error- we don't have anything to hide :)
                    print('['+str(datetime.datetime.now())+'] Error occurred more than number of tries: '+str(e))
                    log_file.write('['+str(datetime.datetime.now())+'] '+str(tweetCriteria.since)+'Error occurred more than number of tries: '+str(e)+'\n')
